...
export async function sendNewPropertySubmissionEmail(
  adminEmail: string,
  ownerName: string,
  ownerEmail: string,
  ownerPhone: string,
  ...
  if (!transporter) {
    console.log(`[EMAIL] Would notify admin ${adminEmail} of new property submission (SMTP not configured)`);
    return false;
  }
  ...
  try {
    await transporter.sendMail({
      from: `"${fromName}" <${fromEmail}>`,
      to: adminEmail,
      subject: `New Property Awaiting Approval: ${propertyTitle}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #ff9a00 0%, #ff7b00 100%); padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 28px;">Leaseo</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 5px 0 0 0;">New Listing Awaiting Approval</p>
          </div>

          <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
            <h2 style="color: #333; margin-top: 0;">Hello Admin,</h2>
            <p>A customer has just submitted a new property. It is currently <strong>pending</strong> and will not appear on the site until it's reviewed.</p>

            <div style="background: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
              <h3 style="margin: 0 0 15px 0; color: #ff9a00;">Property: ${propertyTitle}</h3>
              <p style="margin: 10px 0;"><strong>Submitted by:</strong></p>
              <ul style="list-style: none; padding: 0; margin: 0;">
                <li style="margin: 5px 0;"><strong>Name:</strong> ${ownerName || "N/A"}</li>
                ${ownerEmail ? `<li style="margin: 5px 0;"><strong>Email:</strong> <a href="mailto:${ownerEmail}" style="color: #ff9a00;">${ownerEmail}</a></li>` : ""}
                ${ownerPhone ? `<li style="margin: 5px 0;"><strong>Phone:</strong> ${ownerPhone}</li>` : ""}
              </ul>
            </div>

            <div style="text-align: center; margin: 30px 0;">
              <a href="https://leaseo.in/admin?section=properties&status=pending" style="background: #ff9a00; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold;">Review in Admin Panel</a>
            </div>
...
A new property has been submitted and is pending your approval: ${propertyTitle}

Submitted by: ${ownerName || "N/A"}
${ownerEmail ? `Email: ${ownerEmail}` : ""}
${ownerPhone ? `Phone: ${ownerPhone}` : ""}

Review it here: https://leaseo.in/admin?section=properties&status=pending

---
Leaseo - India's Largest Zero Brokerage Property Site`,
    });

    console.log(`[EMAIL] New property submission notification sent to admin ${adminEmail}`);
    return true;
  } catch (error) {
    console.error(`[EMAIL] Failed to send new property submission notification to ${adminEmail}:`, error);
    return false;
  }
}
...
export async function sendPropertyApprovedEmail(
  ...
  if (!transporter) {
    console.log(`[EMAIL] Would send property approval notice to ${ownerEmail} (SMTP not configured)`);
    return false;
  }
  ...
  try {
    await transporter.sendMail({
      from: `"${fromName}" <${fromEmail}>`,
      to: ownerEmail,
      subject: `Your Property Has Been Approved: ${propertyTitle}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 28px;">Leaseo</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 5px 0 0 0;">Your Listing Is Live</p>
          </div>

          <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
            <h2 style="color: #333; margin-top: 0;">Hello ${ownerName || "there"},</h2>
            <p>Good news! Your property <strong>${propertyTitle}</strong> has been reviewed and approved. It is now live and visible to everyone on Leaseo.</p>

            <div style="text-align: center; margin: 30px 0;">
              <a href="https://leaseo.in/properties/${propertyId}" style="background: #22c55e; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold;">View Your Listing</a>
            </div>
...
Good news! Your property "${propertyTitle}" has been reviewed and approved. It is now live on Leaseo.

View it here: https://leaseo.in/properties/${propertyId}

---
Leaseo - India's Largest Zero Brokerage Property Site`,
    });

    console.log(`[EMAIL] Property approval notice sent to ${ownerEmail}`);
    return true;
  } catch (error) {
    console.error(`[EMAIL] Failed to send property approval notice to ${ownerEmail}:`, error);
    return false;
  }
}
...
export async function sendPropertyRejectedEmail(
  ...
  if (!transporter) {
    console.log(`[EMAIL] Would send property rejection notice to ${ownerEmail} (SMTP not configured)`);
    return false;
  }
  ...
  try {
    await transporter.sendMail({
      from: `"${fromName}" <${fromEmail}>`,
      to: ownerEmail,
      subject: `Update on Your Property Submission: ${propertyTitle}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #64748b 0%, #475569 100%); padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 28px;">Leaseo</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 5px 0 0 0;">Listing Review Update</p>
          </div>

          <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none;">
            <h2 style="color: #333; margin-top: 0;">Hello ${ownerName || "there"},</h2>
            <p>We reviewed your property <strong>${propertyTitle}</strong> and it hasn't been approved for listing at this time.</p>
            ${reason ? `<div style="background: #f5f5f5; padding: 15px; border-radius: 8px; margin: 20px 0;"><strong>Reason:</strong> ${reason}</div>` : ""}
            <p>You're welcome to update the listing details and resubmit, or reach out to our support team if you have questions.</p>
...
We reviewed your property "${propertyTitle}" and it hasn't been approved for listing at this time.
${reason ? `\nReason: ${reason}\n` : ""}
You're welcome to update the listing and resubmit, or contact support if you have questions.

---
Leaseo - India's Largest Zero Brokerage Property Site`,
    });

    console.log(`[EMAIL] Property rejection notice sent to ${ownerEmail}`);
    return true;
  } catch (error) {
    console.error(`[EMAIL] Failed to send property rejection notice to ${ownerEmail}:`, error);
    return false;
  }
}
